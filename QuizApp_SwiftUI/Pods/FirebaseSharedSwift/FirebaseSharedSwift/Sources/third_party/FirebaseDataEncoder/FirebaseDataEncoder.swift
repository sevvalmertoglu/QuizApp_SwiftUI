...
private struct NoPassthroughTypes: StructureCodingPassthroughTypeResolver {
    static func isPassthroughType<T>(_: T) -> Bool {
        return false
    }
}
...
private struct _JSONUnkeyedEncodingContainer: UnkeyedEncodingContainer {
    ...
    public mutating func encode(_ value: Float) throws {
        ...
        defer { self.encoder.codingPath.removeLast() }
        try container.add(encoder.box(value))
    }
...
private extension __JSONEncoder {
    ...
    func box_(_ value: Encodable) throws -> NSObject? {
        ...
        } else if let object = value as? NSObject, options.passthroughTypeResolver.isPassthroughType(value) {
            return object
        }
...
public class FirebaseDataDecoder {
    ...
    open func decode<T: Decodable>(_ type: T.Type, from structure: Any) throws -> T {
        let decoder = __JSONDecoder(referencing: structure, options: options)
...
private class __JSONDecoder: Decoder {
    ...
    public func container<Key>(keyedBy _: Key.Type) throws -> KeyedDecodingContainer<Key> {
        ...
        if let rcValue = storage.topContainer as? FirebaseRemoteConfigValueDecoding {
            guard let top = rcValue.dictionaryValue() else {
                throw DecodingError._typeMismatch(at: codingPath, expectation: [String: Any].self,
                                                  reality: rcValue)
            }
            topContainer = top
        } else {
            guard let top = storage.topContainer as? [String: Any] else {
    ...
    public func unkeyedContainer() throws -> UnkeyedDecodingContainer {
        ...
        if let rcValue = storage.topContainer as? FirebaseRemoteConfigValueDecoding {
            guard let arrayValue = rcValue.arrayValue() else {
                throw DecodingError._typeMismatch(at: codingPath, expectation: [Any].self, reality: rcValue)
            }
...
private struct _JSONKeyedDecodingContainer<K: CodingKey>: KeyedDecodingContainerProtocol {
    ...
    private func _superDecoder(forKey key: __owned CodingKey) throws -> Decoder {
        ...
        return __JSONDecoder(referencing: value, at: decoder.codingPath, options: decoder.options)
    }
...
private extension __JSONDecoder {
    ...
    func rcValNumberAdaptor(_ value: Any) -> Any {
        if let rcValue = value as? FirebaseRemoteConfigValueDecoding {
    ...
    func getNumber(_ value: Any, as type: Any.Type) throws -> NSNumber {
        let val = rcValNumberAdaptor(value)
        guard let number = val as? NSNumber, number !== kCFBooleanTrue, number !== kCFBooleanFalse else {
            throw DecodingError._typeMismatch(at: codingPath, expectation: type, reality: val)
        }
        return number
    }
    ...
    func unbox(_ value: Any, as _: Decimal.Type) throws -> Decimal? {
        ...
        if let decimal = val as? Decimal {
            ...
            let doubleValue = try unbox(val, as: Double.self)!
    ...
    func unbox<T>(_ value: Any, as type: _JSONStringDictionaryDecodableMarker.Type) throws -> T? {
        ...
        if let rcValue = value as? FirebaseRemoteConfigValueDecoding {
            guard let dictionaryValue = rcValue.dictionaryValue() else {
                throw DecodingError._typeMismatch(at: codingPath, expectation: type, reality: rcValue)
            }
    ...
    func unbox_(_ value: Any, as _type: Decodable.Type) throws -> Any? {
        if _type == Date.self || _type == NSDate.self {
            ...
            storage.push(container: value)
            defer { self.storage.popContainer() }
            if options.passthroughTypeResolver.isPassthroughType(value), type(of: value) == _type {
                return value
            }
